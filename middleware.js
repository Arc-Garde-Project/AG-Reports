...
export default async function middleware(request) {
  ...
  if (pathname.includes('..')) return brandedGate(404, PRIVATE_HEADERS);

  // ---- exchange endpoint ----
  if (pathname === '/access/exchange') {
    if (request.method !== 'POST') return brandedGate(404, PRIVATE_HEADERS);

    const origin = request.headers.get('origin');
    const host = request.headers.get('host');
    if (origin) {
      try { if (new URL(origin).host !== host) return brandedGate(404, PRIVATE_HEADERS); }
      catch { return brandedGate(404, PRIVATE_HEADERS); }
    }
    ...
    let body;
    try { body = await request.json(); } catch { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS); }
    const route = typeof body?.route === 'string' ? body.route : '';
    const secret = typeof body?.secret === 'string' ? body.secret : '';
    if (!route || !secret) { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS); }
    ...
    if (!routeKey || !entry || entry.access !== 'PRIVATE_MAGIC_LINK') {
      noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS);
    }

    const tok = tokens()[routeKey];
    const cookieSecret = process.env.COOKIE_SECRET || '';
    if (!tok || !tok.h || !cookieSecret) { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS); }

    const presented = await sha256Hex(secret);
    if (!timingSafeEqual(presented, String(tok.h))) { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS); }
  ...
  if (!routeKey || !entry) return brandedGate(200, GATE_HEADERS);
  if (entry.access !== 'PRIVATE_MAGIC_LINK') return brandedGate(200, GATE_HEADERS);

  const tok = tokens()[routeKey];
  const cookieSecret = process.env.COOKIE_SECRET || '';
  if (!tok || !cookieSecret) return brandedGate(200, GATE_HEADERS);

  const version = Number(tok.v || 1);
  const presented = readCookie(request, cookieNameFor(routeKey));
  if (!presented) return brandedGate(200, GATE_HEADERS);

  const expected = 'v' + version + '.' + (await hmacHex(cookieSecret, routeKey + '|' + version));
  if (!timingSafeEqual(presented, expected)) return brandedGate(200, GATE_HEADERS);
