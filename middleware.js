...
// The gate page is byte-identical for every caller and every route, so it holds
// nothing to keep private and is safe to cache. It also has to be previewable:
// a magic link is shared as a URL, and the unfurler sees THIS page.
//   - noarchive is dropped, because it suppresses snippet generation.
//   - no-store is dropped, because some unfurlers decline to fetch it.
//   - noindex, nofollow stay, so nothing is ever indexed.
const GATE_HEADERS = {
...
// Link-preview title, resolved from the URL NAMESPACE and never from the slug.
//
// That distinction is the whole safety argument. "/presentations/..." versus
// "/reports/..." is already visible in any URL anyone holds, so saying
// Presentation or Report discloses nothing new. The SLUG is the secret, and an
// invented slug gets exactly the card a real one in the same namespace gets, so
// the two remain indistinguishable and the client roster cannot be read off a
// preview.
//
// Longest prefix first, so /presentations wins before any shorter entry could.
const CARD_TITLE = [
  ['/presentations', 'Presentation Made For You And Your Business'],
  ['/reports', 'Report Made For You And Your Business'],
  ['/preview', 'Preview Made For You And Your Business'],
  ['/arborwealth', 'Report Made For You And Your Business'],
  ['/hitch-academy', 'Report Made For You And Your Business'],
];

function cardTitleFor(pathname) {
  for (const [prefix, title] of CARD_TITLE) {
...
export default async function middleware(request) {
  ...
  if (pathname.includes('..')) return brandedGate(404, PRIVATE_HEADERS, pathname);

  // ---- exchange endpoint ----
  if (pathname === '/access/exchange') {
    if (request.method !== 'POST') return brandedGate(404, PRIVATE_HEADERS, pathname);

    const origin = request.headers.get('origin');
    const host = request.headers.get('host');
    if (origin) {
      try { if (new URL(origin).host !== host) return brandedGate(404, PRIVATE_HEADERS, pathname); }
      catch { return brandedGate(404, PRIVATE_HEADERS, pathname); }
    }
    ...
    let body;
    try { body = await request.json(); } catch { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS, pathname); }
    const route = typeof body?.route === 'string' ? body.route : '';
    const secret = typeof body?.secret === 'string' ? body.secret : '';
    if (!route || !secret) { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS, pathname); }
    ...
    if (!routeKey || !entry || entry.access !== 'PRIVATE_MAGIC_LINK') {
      noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS, pathname);
    }

    const tok = tokens()[routeKey];
    const cookieSecret = process.env.COOKIE_SECRET || '';
    if (!tok || !tok.h || !cookieSecret) { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS, pathname); }

    const presented = await sha256Hex(secret);
    if (!timingSafeEqual(presented, String(tok.h))) { noteFailure(ip); return brandedGate(404, PRIVATE_HEADERS, pathname); }
  ...
  if (!routeKey || !entry) return brandedGate(200, GATE_HEADERS, pathname);
  if (entry.access !== 'PRIVATE_MAGIC_LINK') return brandedGate(200, GATE_HEADERS, pathname);

  const tok = tokens()[routeKey];
  const cookieSecret = process.env.COOKIE_SECRET || '';
  if (!tok || !cookieSecret) return brandedGate(200, GATE_HEADERS, pathname);

  const version = Number(tok.v || 1);
  const presented = readCookie(request, cookieNameFor(routeKey));
  if (!presented) return brandedGate(200, GATE_HEADERS, pathname);

  const expected = 'v' + version + '.' + (await hmacHex(cookieSecret, routeKey + '|' + version));
  if (!timingSafeEqual(presented, expected)) return brandedGate(200, GATE_HEADERS, pathname);
